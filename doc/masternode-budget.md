...
NachoCoin now supports full decentralized budgets that are paid directly from the blockchain via superblocks once per month.
...
In this transaction we prepare collateral for "_cool-project_". This proposal will pay _1200_ NACHO, _12_ times over the course of a year totaling _24000_ NACHO.

**Warning: if you change any fields within this command, the collateral transaction will become invalid.**

Format: ```mngovernance prepare proposal-name url payment-count block-start nachocoin-address monthly-payment-nachocoin```
...
Format: ```mngovernance submit proposal-name url payment-count block-start nachocoin-address monthly-payment-nachocoin fee-tx```
...
When block ```1000000``` is reached you'll receive a payment for ```1200``` NACHO to ```y6R9oN12KnB9zydzTLc3LikD9cCjjQzYG7```.
...
The following RPC commands are supported:
 ...
 - mngovernance "command"... ( "passphrase" )
  ...
  - vote               - Vote on a proposal by single masternode (using nachocoin.conf setup)
