...
void BitcoinGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a NachoCoin address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and nachocoin: URIs)"));
    ...
    aboutAction->setStatusTip(tr("Show information about NachoCoin Core"));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your NachoCoin addresses to prove you own them"));
    verifyMessageAction = new QAction(QIcon(":/icons/" + theme + "/transaction_0"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified NachoCoin addresses"));
    ...
    openAction->setStatusTip(tr("Open a nachocoin: URI or payment request"));

    showHelpMessageAction = new QAction(QApplication::style()->standardIcon(QStyle::SP_MessageBoxInformation), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible NachoCoin command-line options").arg(tr(PACKAGE_NAME)));
...
void BitcoinGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        labelConnectionsIcon->setToolTip(tr("%n active connection(s) to NachoCoin network", "", count));
    } else {
...
{
    QString strTitle = tr("NachoCoin Core"); // default title
    ...
    // Append title to "NachoCoin Core - "
    if (!msgType.isEmpty())
        strTitle += " - " + msgType;
