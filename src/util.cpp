...
const char * const BITCOIN_CONF_FILENAME = "nachocoin.conf";
const char * const BITCOIN_PID_FILENAME = "nachocoind.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            if (mapMultiArgs.count("-debug")) {
                ...
                // "nachocoin" is a composite category enabling all NachoCoin-related debug output
                if(ptrCategory->count(std::string("nachocoin"))) {
...
#else
    const char* pszModule = "nachocoin";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\NachoCoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\NachoCoin
    // Mac: ~/Library/Application Support/NachoCoin
    // Unix: ~/.nachocoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "NachoCoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/NachoCoin";
#else
    // Unix
    return pathRet / ".nachocoin";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty nachocoin.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override nachocoin.conf
