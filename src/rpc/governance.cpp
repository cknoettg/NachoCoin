...
UniValue gobject(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp  ||
        ...
        throw std::runtime_error(
                ...
                "  vote-conf          - Vote on a governance object by masternode configured in nachocoin.conf\n"
    ...
    */
    ...
    if(strCommand == "vote-conf")
    {
        ...
        if(!fMnFound) {
            ...
            resultsObj.push_back(Pair("nachocoin.conf", statusObj));
...
{ //  category              name                      actor (function)         okSafe argNames
  //  --------------------- ------------------------  -----------------------  ------ ----------
    /* NachoCoin features */
    { "nachocoin",               "getgovernanceinfo",      &getgovernanceinfo,      true,  {} },
    { "nachocoin",               "getsuperblockbudget",    &getsuperblockbudget,    true,  {"index"} },
    { "nachocoin",               "gobject",                &gobject,                true,  {} },
    { "nachocoin",               "voteraw",                &voteraw,                true,  {} },
